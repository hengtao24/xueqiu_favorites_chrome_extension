...
- 自动规则分组的更多维度（作者、来源、正则匹配）

> 已实现（原列于本节，现已完成）：分组拖拽排序、分组重命名、分组名重复校验、右键 Tab 重命名/删除菜单、存储配额提示、跨设备同步（`chrome.storage.sync`）、自动规则分组（股票代码/关键词，详见 `2026-06-02-auto-grouping-design.md`）。
